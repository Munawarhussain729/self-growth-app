...
export function TodayScreen({
	...
	const existing = state.journal.find(
		(entry) => entry.createdAt.slice(0, 10) === todayKey(),
	);
			...
			updateState({
				...
				journal: state.journal.map((entry) =>
					entry.id === existing.id
						? { ...entry, answer: trimmed, updatedAt: now }
						: entry,
				),
			});
			return;
	...
	return (
		<View>
			<View style={styles.hero}>
				...
				<View style={styles.heroTop}>
					...
					<View style={styles.statusPill}>
						<Icon
							name={savedToday ? "checkmark-circle" : "ellipse-outline"}
							size={16}
							color="#FFFFFF"
						/>
				...
				<Text style={styles.heroSubtitle}>
					One focused idea, one honest reflection, one small action.
				</Text>
			...
			<Card style={styles.promptCard}>
				...
				/>
				<Button
					label={savedToday ? "Update Reflection" : "Save Reflection"}
					onPress={save}
